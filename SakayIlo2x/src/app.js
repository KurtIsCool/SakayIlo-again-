import { mockRoutesDB } from './mockData.js';
...
mockRoutesDB.features.forEach(feature => {
...
// 3. UI State Setup
const drawnLines = L.layerGroup().addTo(map); // Layer group to easily clear previous routes
...
const startMarker = L.marker([10.722, 122.556], { draggable: true, icon: startIcon }).addTo(map);
const endMarker = L.marker([10.6974, 122.5644], { draggable: true, icon: endIcon }).addTo(map);
...
function calculateRoute() {
  ...
  try {
    ...
    console.log('[DEBUG] Nearby Start Routes found:', startNearby.map(n => n.route.properties.name));
    console.log('[DEBUG] Nearby End Routes found:', endNearby.map(n => n.route.properties.name));
    ...
    if (bestDirect) {
      console.log('[SUCCESS] Direct route found:', bestDirect.s.route.properties.name);
      ...
      resultsDiv.innerHTML = `
        ...
        <div class="step">🚶 Walk ${Math.round(bestDirect.s.distance)}m to ${bestDirect.s.route.properties.name}</div>
        <div class="step" style="border-left: 4px solid ${bestDirect.s.route.properties.color}">🚙 Ride ${Math.round(bestDirect.rideDist)}m on ${bestDirect.s.route.properties.name}</div>
    ...
    if (bestTransfer) {
      console.log(`[SUCCESS] Transfer route found: ${bestTransfer.s.route.properties.name} -> ${bestTransfer.e.route.properties.name}`);
      ...
      resultsDiv.innerHTML = `
        ...
        <div class="step">🚶 Walk ${Math.round(bestTransfer.s.distance)}m to ${bestTransfer.s.route.properties.name}</div>
        <div class="step" style="border-left: 4px solid ${bestTransfer.s.route.properties.color}">🚙 Ride ${Math.round(bestTransfer.ride1Dist)}m on ${bestTransfer.s.route.properties.name}</div>
        <div class="step">🔄 Transfer to ${bestTransfer.e.route.properties.name}</div>
        <div class="step" style="border-left: 4px solid ${bestTransfer.e.route.properties.color}">🚙 Ride ${Math.round(bestTransfer.ride2Dist)}m on ${bestTransfer.e.route.properties.name}</div>
